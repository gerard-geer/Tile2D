...
int main(int argc, char **argv)
{
    ...
    for(int i = 0; i < 4; ++i)
    {
        for(int j = 0; j < 16;  ++j)
        {
            tpwater[(i*16)+j] = r->makeAnimTile(x,y,PLANE_PLAYFIELD_A,32,32,true,"water",4,64,64,1.0/12.5);
            y += 32;
        }
        x += 32;
        y = 128;
    ...
    for(int i = 0; i < 2; ++i)
    {
        lwsplash[i] = r->makeAnimTile(x,64,PLANE_PLAYFIELD_C,32,16,true,"splash",4,32,16,1/12.5);
        x+=16;
    }
    ...
    for(int i = 0; i < 7; ++i)
    {
        ...
        tpsplash[i] = r->makeAnimTile(x,192,p,32,16,true,"splash",4,32,16,1/12.5);
        x+=16;
    }
    x = 72;
    for(int i = 0; i < 10; ++i)
    {
        tpsplash[i+7] = r->makeAnimTile(x,64,PLANE_PLAYFIELD_C,32,16,true,"splash",4,32,16,1/12.5);
        x+=16;
    }
    ...
    {
        tile_plane p = (i%2==0)?PLANE_PLAYFIELD_A:PLANE_PLAYFIELD_C;
        lpsplash[i] = r->makeAnimTile(x,128,p,32,16,true,"splash",4,32,16,1/12.5);
        x+=16;
    }
    x = 272;
    for(int i = 0; i < 15; ++i)
    {
        lpsplash[i+7] = r->makeAnimTile(x,64,PLANE_PLAYFIELD_C,32,16,true,"splash",4,32,16,1/12.5);
        x+=16;
    }
